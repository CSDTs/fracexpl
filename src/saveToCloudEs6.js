 ...
 */
export default class CloudSaver {
    constructor(
        optionalProjAPIURL,
        optionalFileAPIURL,
        optionalLoginUrl,
    ...
    @param {function} errorCallBack - If there is an error
     */
    saveFile(file, callBack, errorCallBack) {
      $.ajax({
        type: 'PUT',
        url: this.fileAPIURL,
        data: file,
    ...
    @param {function} errorCallBack - If there is an error
     */
    createProject(projectName, applicationID, dataID, imgID, callBack, errorCallBack) {
    ...
    @param {function} errorCallBack - If there is an error
     */
    loadProject(projectID, callBack, errorCallBack) {
        $.get(`${this.projAPIURL + projectID}/`, null, data => {
    ...
    @param {function} errorCallBack - If there is an error
     */
    listProject(userID, callBack, errorCallBack) {
    ...
    @param {function} errorCallBack - If there is an error
     */
    getGISDatasets(callBack, errorCallBack) {
       this.getCSRFToken();
       $.ajax({
          dataType: 'json',
          url: this.gisDSURL,
    ...
    getGISPolys(
        ...
        optionalTags) {
       this.getCSRFToken();
       let query = `${this.gisPolyURL}?dataset=${dataset}&min_lat=${minLat}&max_lat=${maxLat}&min_lon=${minLong}&max_lon=${maxLong}`;
       if (optionalTags) {
    ...
    getGISPoints(
        ...
        optionalTags) {
       this.getCSRFToken();
       let query = `${this.gisPointURL}?dataset=${dataset}&min_lat=${minLat}&max_lat=${maxLat}&min_lon=${minLong}&max_lon=${maxLong}`;
       if (optionalTags) {
         query += `?tags=${optionalTags}`;
       }
       $.ajax({
