...
import CloudSaver from '/src/saveToCloudEs6.js';
import {saveAs} from '/src/FileSaver.js';
...
*/
class FractalDraw {
  constructor(toolNum, seed, askWidth, askHeight, levels, instanceNum) {
    this.seed = seed;
  ...
  loadRemotely(evt) {
    ...
    function displayList(data) {
      const dialogDiv = $('#projectListDialog');
  ...
  drawSeed(drawBaseLine = false, without = -1) {
    this.clear();
  ...
  basedraw(start, end, hflip, level) {
...
*/
class SeedEditor {
  constructor(fractalDraw, enabled) {
  ...
  addSeed(name, longname, seed, width) {
...
*/
class MultiModeTool {
  constructor(mainDiv, toolNum, askWidth, askHeight, instanceNum) {
    this.instanceNum = instanceNum;
  ...
  addMode(title, modeObj, globalId) {
    let button = document.createElement('button');
